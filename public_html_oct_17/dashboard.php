...
if(isset($_POST['submit']))
{
 ...
 echo $countfiles = count($_FILES['image']['name']);
 // Looping all files
 for($i=0;$i<$countfiles;$i++){
  //$filename="";
  echo  $filename = time()."-".$_FILES['image']['name'][$i];
   
   // Upload file
   if(move_uploaded_file($_FILES['image']['tmp_name'][$i],'../unitimages/'.$filename)){
   $ts = time();
		//$imageFileName = ""; 
		//$imageFileName = time() . "_" . $name;
		//move_uploaded_file($_FILES['image']['tmp_name'], '../unitimages/' . $imageFileName);
		$location = '../unitimages/' .$filename;
			$options ="";
